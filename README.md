...
- 🎯 Entry points to execute your program using `python -m <e_commerce_back_end>` or `$ e_commerce_back_end` with basic CLI argument parsing.
...
---
# e_commerce_back_end Flask Application

Awesome e_commerce_back_end created by YUeQiii
...
```bash
git clone https://github.com/YUeQiii/e-commerce-back-end e_commerce_back_end
cd e_commerce_back_end
make install
```
...
```bash
pip install e_commerce_back_end
```
...
```bash
e_commerce_back_end create-db   # run once
e_commerce_back_end populate-db  # run once (optional)
e_commerce_back_end add-user -u admin -p 1234  # ads a user
e_commerce_back_end run
```
